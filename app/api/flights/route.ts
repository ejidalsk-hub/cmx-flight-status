...
function buildAviationstackUrl(kind: 'arrival' | 'departure', date: string) {
  ...
  // Free tier behaves better for current-day realtime calls.
  if (date !== today) {
...
async function fetchAviationstackList(kind: 'arrival' | 'departure', date: string) {
  const response = await fetch(buildAviationstackUrl(kind, date), {
    next: { revalidate: 300 },
  });
...
async function fetchAviationstack(date: string): Promise<FlightsPayload> {
  ...
  try {
    arrivalsData = await fetchAviationstackList('arrival', date);
  } catch (error) {
    arrivalsError = error instanceof Error ? error.message : 'Unknown arrivals error';
  }

  try {
    departuresData = await fetchAviationstackList('departure', date);
  } catch (error) {
    departuresError = error instanceof Error ? error.message : 'Unknown departures error';
  }

  // Only fallback completely if both calls failed.
  if (!arrivalsData.length && !departuresData.length) {
    return {
      ...getDemoFlights(date),
      source: 'demo fallback',
      note: isToday
        ? 'Both live flight API calls failed for today. Showing fallback rows instead.'
        : 'Date-based lookups failed on the provider free plan. Showing fallback rows instead.',
      error: `Arrivals: ${arrivalsError ?? 'none'} | Departures: ${departuresError ?? 'none'}`,
    };
  }
  ...
  // If the provider returned data but filtering removed everything, fall back.
  if (!payload.arrivals.length && !payload.departures.length) {
    return {
      ...getDemoFlights(date),
      source: 'demo fallback',
      note: isToday
        ? 'Provider returned no flights that passed the CMX filter. Showing fallback rows instead.'
        : 'This free provider plan does not reliably support date-based CMX flight lookups. Showing fallback rows instead.',
      error: `Filtered out all provider results. Raw counts - arrivals: ${arrivalsData.length}, departures: ${departuresData.length}`,
    };
  }
  ...
  if (payload.arrivals.length < 2 || payload.departures.length < 2) {
    ...
    const notes: string[] = [];
    if (arrivalsError) notes.push(`Arrivals issue: ${arrivalsError}`);
    if (departuresError) notes.push(`Departures issue: ${departuresError}`);

    payload.note = isToday
      ? `Only flights matching likely CMX-ORD United/SkyWest service were kept. Fallback rows were added because fewer than 2 valid live flights were returned.${notes.length ? ' ' + notes.join(' | ') : ''}`
      : 'Date-based lookup returned limited data on the free plan, so fallback rows were added.';
  }

  if (arrivalsError || departuresError) {
    payload.error = `Arrivals: ${arrivalsError ?? 'ok'} | Departures: ${departuresError ?? 'ok'}`;
  }
